...
const SERVER_NAME = "mediaclaw-codex-bridge";
...
const BRIDGE_PROXY_CHECK_INTERVAL_MS = 5_000;
...
const METHOD_REGISTRY = JSON.parse(
  fs.readFileSync(
    new URL("../contracts/methods-v1.json", import.meta.url),
    "utf8",
  ),
);
const deviceIdentity = await loadOrCreateDeviceIdentity({
  host: process.env.MEDIACLAW_AGENT_HOST || "codex",
  displayName:
    process.env.MEDIACLAW_AGENT_DEVICE_NAME || "MediaClaw Agent (Codex)",
});
...
let bridgeProxyMode = false;
let bridgePromotionInFlight = false;
...
function createTask({
  ...
  const task = {
    taskId,
    kind,
    status: extensionPeer && extensionInfo ? "queued" : "waiting_for_extension",
    message: extensionPeer && extensionInfo
      ? "等待浏览器插件执行"
      : extensionPeer
...
function pumpQueue() {
  if (!extensionPeer || !extensionInfo || activeExtensionTaskId) return;
...
function acceptTaskResultMessage(message = {}) {
  ...
  if (!task) {
    task = recoverTaskFromResult(taskId, message);
  ...
  if (extensionInfo?.pendingResultCount > 0) {
    extensionInfo.pendingResultCount -= 1;
  }
  extensionPeer?.send({
...
function handleExtensionMessage(message = {}) {
  if (message.type === "session.challenge") {
    ...
    extensionPeer?.send({
      ...
      deviceId,
      challengeId,
      signature: deviceIdentity.sign(payload),
    });
    return;
  }
  if (message.type === "extension.hello") {
    if (
      ...
      normalizeText(message.deviceId, 160) !== deviceIdentity.deviceId ||
      !normalizeText(message.sessionId, 200)
    ) {
      return;
    }
    extensionInfo = {
...
async function cancelTask(taskId) {
  ...
  if (activeExtensionTaskId === task.taskId && extensionPeer) {
    extensionPeer.send({type: "task.cancel", taskId: task.taskId});
    activeExtensionTaskId = "";
  }
  task.resolveCompletion(task);
  pumpQueue();
...
async function proxyToolCall(params = {}) {
  const timeoutMs =
    String(params.name || "") === "mediaclaw_connection_status"
      ? 10_000
      : LEGACY_WAIT_TIMEOUT_MS + 10_000;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch(`${BRIDGE_HTTP_ORIGIN}/rpc`, {
      method: "POST",
    ...
    if (!response.ok) {
      throw new Error(`bridge proxy returned HTTP ${response.status}`);
    }
    return await response.json();
  } catch (error) {
    await tryPromoteProxyBridge();
    if (!bridgeProxyMode) {
      return handleToolCall(params, {allowProxy: false});
    }
    return toolResult(
      {
        ok: false,
        error: {
          code: "BRIDGE_PROXY_UNAVAILABLE",
          message: `MediaClaw 主桥接暂时不可用：${
...
async function handleToolCall(params = {}, {allowProxy = true} = {}) {
  if (allowProxy && bridgeProxyMode) {
    return proxyToolCall(params);
  }
  const name = String(params.name || "");
  const args = params.arguments || {};

  if (name === "mediaclaw_connection_status") {
    const connected = Boolean(extensionPeer && extensionInfo);
    const awaitingPairing = Boolean(extensionPeer && !extensionInfo);
  ...
  if (!extensionPeer || !extensionInfo) {
...
async function handleBridgeHttpRequest(request, response) {
  ...
  const result = await handleToolCall(payload.params || {}, {
    allowProxy: false,
  });
...
async function hasHealthyPrimaryBridge() {
  const controller = new AbortController();
  ...
  try {
    ...
    return payload?.service === "mediaclaw-codex-bridge";
  } catch {
    return false;
  } finally {
    clearTimeout(timeout);
...
function setPrimaryBridgeStatus() {
  bridgeProxyMode = false;
  bridgeStatus = {
    listening: true,
    host: "127.0.0.1",
    port: DEFAULT_PORT,
...
function setProxyBridgeStatus() {
  bridgeProxyMode = true;
  bridgeStatus = {
    listening: true,
    host: "127.0.0.1",
    port: DEFAULT_PORT,
...
async function tryPromoteProxyBridge() {
  if (!bridgeProxyMode || bridgePromotionInFlight) {
    return;
  }
  bridgePromotionInFlight = true;
  try {
    if (await hasHealthyPrimaryBridge()) {
      return;
    }
    try {
      await websocketServer.listen();
      setPrimaryBridgeStatus();
      process.stderr.write(
        `[mediaclaw] promoted to primary bridge on 127.0.0.1:${DEFAULT_PORT}\n`,
      );
    } catch (error) {
      if (await hasHealthyPrimaryBridge()) {
        setProxyBridgeStatus();
        return;
      }
      bridgeStatus = {
        listening: false,
  ...
  try {
    if (message.method === "initialize") {
      ...
      writeResult(message.id, {
        ...
        instructions:
          "先检查连接。找选题趋势用 mediaclaw_research_keyword_topics，找长尾需求用 mediaclaw_research_longtail_keywords，找博主近期爆款用 mediaclaw_research_account_hits，找对标账号用 mediaclaw_research_benchmark_accounts，拆解单篇用 mediaclaw_research_single_note；按已保存账号风格创作时必须先用 mediaclaw_get_style_profile 读取真实分析，找不到时禁止猜测。视频逐字稿必须先调用报价工具，向用户展示预计积分并取得明确同意后，再用一次性 quoteId 确认；V0.3 不支持 BYOK 或本地转录。MediaClaw 负责准备数据，最终分析由当前 Agent 按 Skill 方法完成。",
      });
      return;
    }
    ...
    if (message.method === "tools/call") {
      writeResult(message.id, await handleToolCall(message.params || {}));
      return;
    }
    if (message.method === "tasks/get") {
      const task = tasks.get(String(message.params?.taskId || ""));
      if (!task) {
        writeError(message.id, -32001, "Task not found");
      } else {
        writeResult(message.id, {task: taskSnapshot(task)});
      }
      return;
    }
    ...
    if (message.method === "tasks/cancel") {
      const result = await cancelTask(String(message.params?.taskId || ""));
      if (!result.ok) {
        writeError(message.id, -32001, result.error.message);
      } else {
        writeResult(message.id, {task: result.task});
      }
      return;
    }
    if (message.method === "tasks/result") {
      ...
      if (!task) {
        ...
        writeError(message.id, -32002, "Task is not complete");
      } else {
        writeResult(
          message.id,
          toolResult(getTaskPublicResult(task), {
...
const websocketServer = createLoopbackWebSocketServer({
  ...
  onConnection(peer) {
    ...
    peer.send({
      type: "server.hello",
      serverName: "MediaClaw Codex Bridge",
...
try {
  ...
  setPrimaryBridgeStatus();
  process.stderr.write(
    `[mediaclaw] local Agent bridge listening on 127.0.0.1:${DEFAULT_PORT}\n`,
  );
} catch (error) {
  if (await hasHealthyPrimaryBridge()) {
    setProxyBridgeStatus();
    process.stderr.write(
      `[mediaclaw] proxying through existing bridge on 127.0.0.1:${DEFAULT_PORT}\n`,
    );
  } else {
    bridgeStatus = {
      listening: false,
    ...
    process.stderr.write(
      `[mediaclaw] cannot start local Agent bridge: ${bridgeStatus.error}\n`,
    );
  }
}

bridgeMonitorTimer = setInterval(() => {
  void tryPromoteProxyBridge();
}, BRIDGE_PROXY_CHECK_INTERVAL_MS);
bridgeMonitorTimer.unref?.();
